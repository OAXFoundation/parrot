...
//! # Burner Module
//! This is the OAX Auto Burn feature. A portion of transaction fees are send
//! to the burn pot and this pot is burnt automatically every BurnPeriod.
//! This burn pot account has no private key, therefore nobody is able
//! to use these funds.
use frame_support::{
...
decl_storage! {
    ...
    // Extra genesis so the account is started with min balance during
    // genesis to avoid dust collection
    add_extra_genesis {
        build(|_config| {
            // Create Burn account
...
mod tests {
    ...
    fn burn_works() {
        new_test_ext().execute_with(|| {
            ...
            // run burn
            Burner::burn();
            // make sure pot returns 0 after burning
            assert_eq!(Burner::pot(), 0);
    ...
    fn existential_deposit_amount() {
        new_test_ext().execute_with(|| {
            let burn_amount = 200;
            Balances::make_free_balance_be(&Burner::account_id(), burn_amount);
            // burn cannot leave the balance totally empty due to existential
            // deposit requirement so we expect there to be 1 token left
            // after burning so test that it doesn't burn everything
