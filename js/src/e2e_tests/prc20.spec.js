...
describe('prc20', () => {
  ...
  test('createToken in setup worked correctly and new token and mints to user', async () => {
    ...
    let aliceTokenBalance = await parrot.getTokenBalance(ALICE.address, tokenIdAlice);
    let bobTokenBalance = await parrot.getTokenBalance(BOB.address, tokenIdBob);
  ...
  test('transferToken works', async () => {
    let bobTokenBalanceInit = await parrot.getTokenBalance(BOB.address, tokenIdAlice);
    // try token transfer
    await parrot.transferToken(ALICE, BOB.address, tokenIdAlice, AMOUNT);
    await sleep(SLEEP);
    // make sure balances updated
    let bobTokenBalance = await parrot.getTokenBalance(BOB.address, tokenIdAlice);
  ...
  test('tokenApprove works', async () => {
    ...
    let tokenApprovedAmount = await parrot.getAllowanceOf(ALICE.address, BOB.address, tokenIdAlice);
    expect(tokenApprovedAmount.toNumber()).toBe(AMOUNT);
  });

  test('transferFrom works', async () => {
    // bob bal init
    let tokenBalBobInit = await parrot.getTokenBalance(BOB.address, tokenIdAlice);
    ...
    await sleep(SLEEP);
    // check bob bal
    let tokenBalBob = await parrot.getTokenBalance(BOB.address, tokenIdAlice);
  ...
  test('multiTransfer for Token Works', async () => {
    ...
    let td1 = { amount: AMOUNT, to: BOB.address };
    let td2 = { amount: AMOUNT, to: CHARLIE.address };
    let mtvec = [td1, td2];
